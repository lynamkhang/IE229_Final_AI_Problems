...
Original file is located at
    https://colab.research.google.com/drive/1ESjpldG6abH_ibOO4heyDiPAbBmxdaG1
"""
...
def padding(bb, vocabidx):
    for tokenlists, labels in bb:
        maxlen = max([len(x) for x in tokenlists])
        for i in range(len(tokenlists)):
            tokenlists[i] = tokenlists[i] + ['<pad>'] * (maxlen - len(tokenlists[i]))
    return bb

train_data = padding(train_data, vocabidx)
test_data = padding(test_data, vocabidx)
for i in range(10):
  print(train_data[i])
...
class LSTM(torch.nn.Module):
    def __init__(self):
        ...
        #LSTM with 2 layers and 516 hidden units
        self.l1 = torch.nn.LSTM(300,516,2,dropout=0.5)
        #Linear layer
        self.l2 = torch.nn.Linear(516*2,2)

    def forward(self, x):
        embedded = self.emb(x)
        #LSTM processing
        output, (hidden, cell) = self.l1(embedded)
